...
function createCalendarEventForProduct(eventName, startTime, eventType, productId) {
  try {
...
function deleteCalendarEventForProduct(productId) {
  try {
    ...
    const calEventId = product.meta_data.find(m => m.key === 'google_cal_event_id')?.value;
    
    if (calEventId) {
      CalendarApp.getCalendarById(CALENDAR_ID).getEventById(calEventId).deleteEvent();
