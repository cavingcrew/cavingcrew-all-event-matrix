...
function getProductById(productId) {
	...
	if (response.getResponseCode() === 404) {
	  ...
	  console.log(`[DEBUG] Retrieved product ${productId}`, 
		`SKU: ${productData.sku}`, 
...
function createDuplicateProduct(originalProduct) {
	...
	console.log(`[DUPLICATE] Cloning product ${originalProduct.id}`, 
...
function copyMembershipDiscounts(sourceProductId, newProductId) {
	console.log(`[MEMBERSHIP] Copying discounts from ${sourceProductId} to ${newProductId}`);
	...
	for (const plan of plans) {
		console.log(`[MEMBERSHIP] Processing plan ${plan.id} "${plan.name}"`, 
		...
		console.log(`[MEMBERSHIP] ${updated ? 'Success' : 'Failed'} updating plan ${plan.id}`, 
...
function sendProductToWordPress(product) {
	console.log(`[PRODUCT CREATE] Attempting to create product from template:`, 
	  JSON.stringify({
		name: product.name,
		type: product.type,
		sku: product.sku,
	...
	if (product.variations && product.variations.length > 0) {
		console.log(`[VARIATION CREATE] Creating ${product.variations.length} variations for product ${newProduct.id}`);
		const variationsUrl = `https://www.${apidomain}/wp-json/wc/v3/products/${newProduct.id}/variations`;
		
		product.variations.forEach((v, i) => {
		  console.log(`[VARIATION ${i}]`, `Attributes: ${JSON.stringify(v.attributes)}`, 
			`SKU: ${v.sku}`, `Stock: ${v.stock_quantity}`);
		});
